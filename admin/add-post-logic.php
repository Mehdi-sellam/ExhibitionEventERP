<?php
require 'config/database.php';

if (isset($_POST['submit'])) {
    $author_id = $_SESSION['user_id'];
    $title = filter_var($_POST['title'], FILTER_SANITIZE_FULL_SPECIAL_CHARS);
    $body = filter_var($_POST['body'], FILTER_SANITIZE_FULL_SPECIAL_CHARS);
    $category_id = filter_var($_POST['category'], FILTER_SANITIZE_NUMBER_INT);
    $is_featured = filter_var($_POST['is_featured'], FILTER_SANITIZE_NUMBER_INT);
    $thumbnail = $_FILES['thumbnail'];
    $eventdt = $_POST['eventdt'];

    //set is_featuredto 0 if unchecked
    $is_featured = $is_featured == 1 ?: 0;

    //validate form data
    if (!$title) {
        $_SESSION['add-post'] = "Enter post title";
    } elseif (!$category_id) {
        $_SESSION['add-post'] = "Select post category";
    } elseif (!$body) {
        $_SESSION['add-post'] = "Enter post body";
    // } elseif (!$eventdt) {
    //     $_SESSION['add-post'] = "Enter date and time of Event";
    } elseif (!$thumbnail['name']) {
        $_SESSION['add-post'] = "Choose post thumbnail";
    } else {
        //thumbnail
        //rename image
        $time = time();
        $thumbnail_name = $time . $thumbnail['name'];
        $thumbnail_tmp_name = $thumbnail['tmp_name'];
        $thumbnail_destination_path = '../image/' . $thumbnail_name;

        //check if is image
        $allowed_files = ['png', 'jpg', 'jpeg', 'webp'];
        $extention = explode('.', $thumbnail_name);
        $extention = end($extention);
        if (in_array($extention, $allowed_files)) {
            //check size (2mb+)
            if ($thumbnail['size'] < 2000000) {
                //upload thumbnail
                move_uploaded_file($thumbnail_tmp_name, $thumbnail_destination_path);
            } else {
                $_SESSION['add-post'] = "Reduce the file size";
            }
        } else {
            $_SESSION['add-post'] = "File extention not valid, use: png, jpg, jpeg or webp";
        }
    }

    //check for error
    //move back to register page in case of problem
    if (isset($_SESSION['add-post'])) {
        // pass form data back to regidtration page
        $_SESSION['add-post-data'] = $_POST;
        header('location: ' . ROOT_URL . '/admin/add_post.php');
        die();
    } else {
        //set is_featured of all post to 0 if is_featured for this post is 1
        if ($is_featured == 1) {
            $zero_all_is_featured_query = "UPDATE post SET is_featured=0";
            $zero_all_is_featured_result = mysqli_query($connection, $zero_all_is_featured_query);
        }

        //inser post to database
        $query = "INSERT INTO post (title,body, thumbnail, category_id, author_id, is_featured, eventdt) VALUES ('$title', '$body', '$thumbnail_name', $category_id, $author_id, $is_featured, '$eventdt')";
        $result = mysqli_query($connection, $query);

        if (!mysqli_errno($connection)) {
            $_SESSION['add-post-success'] = "New post added successfully!";
            header('location: ' . ROOT_URL . 'admin/');
            die();
        }
    }
}
header('location: ' . ROOT_URL . 'admin/add_post.php');
die();
